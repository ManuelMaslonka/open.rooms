...
public class RoomRestController {
    ...
    public record CreateReq(String roomId) {
    }

    public record CreateResp(String roomId) {
    }
    ...
    public CreateResp create(@RequestBody() CreateReq req) {
        ...
        if (store.exists(roomId)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "room already exists");
        }

        Room room = Room.create(roomId);
        store.create(room);

        CreateResp createResp = new CreateResp(roomId);
    ...
    public Map<String, Object> get(@PathVariable String roomId) {
        ...
        return Map.of("roomId", r.id(), "online", r.participants().size());
    }

    @GetMapping
    public Map<String, Object> info() {
        return Map.of("rooms", store.count(), "activeRooms", store.activeCount());
    }
}
